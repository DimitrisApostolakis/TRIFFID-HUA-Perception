# TRIFFID Perception — Partner Integration Build

Minimal build of the TRIFFID perception modules for integration testing.
Two independent modules, each in its own Docker container, both running on
**live data only**:

| Module | Input | Output |
|---|---|---|
| **UGV** (`triffid_ugv_perception`, ROS 2 Humble) | Live RGB-D camera topics + GPS/odometry | `Detection3DArray` + GeoJSON on ROS topics; periodic GeoJSON snapshot to disk and/or HTTP PUT to the TRIFFID mapping API |
| **UAV** (`triffid_uav_perception`, plain Python) | Drone video (MP4 + DJI SRT telemetry sidecar), polled live from the FUTURISED API | One pixel-space GeoJSON per video, saved to disk; optional upload to the TELESTO backend |

There are no wrapper scripts on this branch — every command below is run
directly, exactly as written. Full message/field-level specification:
...
ByteTrack IDs, GeoJSON geo-referencing via GPS + heading.
...
Set the three topic parameters to wherever **your** camera publishes. The
...
(0.5), tracker tuning (`tracker_*` — see INTERFACE.md).
...
| `publish_to_api` | `false` | Every period, HTTP `PUT` **one feature per request** (`{"geometry": …, "properties": …}`) to `api_url` |
| `publish_period_s` | `5.0` | Seconds between periodic flushes |
| `output_dir` | `/ws/samples` | Snapshot directory inside the container |
| `api_url` | `https://crispres.com/wp-json/map-manager/v1/features` | TRIFFID mapping API endpoint |
...
|---|---|---|
| in | *(your RGB / depth / CameraInfo topics)* | `sensor_msgs/Image`, `CameraInfo` |
...
Drone-video detection producing **pixel-space** GeoJSON: YOLO-seg with
persistent ByteTrack IDs; each detection reduced to a few independent
pixel sample points (`Point`/`MultiPoint` — never an outline); DJI SRT
...
### Live mode — poll the FUTURISED API

Polls for newly uploaded videos (`.mp4`/`.mov`) **and their `.srt`
telemetry sidecars**, processes each video as it arrives, and writes one
GeoJSON per video:
...
backend (off by default; meaningful once coordinates are geographic).

### Local-file test mode

To verify the module against a file you already have (place it under
`./uav_videos/`, with its `.srt` next to it — auto-detected by name):
...
<sub>* the CLI default is `best.pt` relative to the workdir, which
resolves to `/app/best.pt` in the container.</sub>
...
- `drone_*` / `gimbal_*` describe the **camera** at that frame (from the
  SRT), not the detection; they are omitted when no SRT is available.

---

## 3. Backend posting — the complete picture

Nothing on this branch contacts any backend unless you turn it on:

- **UGV**: `publish_to_api:=true` on `geojson_bridge` → periodic
  per-feature `PUT` to `api_url`. That is the only UGV network output.
- **UAV**: `--post-telesto` → after each video, per-feature `PUT` to the
  TELESTO Map Manager (via `src/triffid_telesto/telesto_client.py`;
  override the endpoint with `--telesto-base-url` / env
  `TELESTO_BASE_URL`).

With both off (the defaults), all outputs stay local: ROS topics +
`./samples/ugv_detections.geojson` for the UGV, and
`./uav_samples/*.geojson` for the UAV.
...
├── classes.txt                               # 63-class ontology + GeoJSON types
├── INTERFACE.md                              # full interface specification
└── src/
